...
def process_polygon(polygon_row, las_files_directory, output_folder, max_pts):
    ...
    if os.path.exists(las_file_path):
        sample_points_within_polygon(las_file_path, polygon, max_pts, output_folder, poly_id, get_attributes=True)
    else:
...
def sample_pts(polygons_file_path, las_files_directory, output_folder, max_pts):
    ...
    with Pool(processes=num_cores) as pool:
        pool.starmap(process_polygon, [(polygon_row, las_files_directory, output_folder, max_pts) for _, polygon_row in gdf_polygons.iterrows()])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_pts', type=int, default=7168, help="Maximum number of points to sample.")
    
    args = parser.parse_args()
    
    sample_pts(args.polygons_file_path, args.las_files_directory, args.output_folder, args.max_pts)